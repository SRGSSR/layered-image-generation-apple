...
var PngGenerator = function (generator, document, setup, useSuffixesInFilenames) {
    ...
    this.JSON_PROFILE_P3 = 'displayp3';
...
PngGenerator.prototype.getAppliedScale = function () {
    var rootLayerName = this.getRootLayer().name.toLowerCase();
    if (rootLayerName === this.ROOT_LAYER_ART) {
        return 1;
    } else if (rootLayerName === this.ROOT_LAYER_ART_P3) {
...
PngGenerator.prototype._generateMeta = function (preview, done, progress) {
    ...
    var scaleToApply = this.getAppliedScale();
    ...
    var lsrMeta = {
        ...
        properties: {
            canvasSize: {
                width: Math.round(this.document.bounds.right / scaleToApply),
                height: Math.round(this.document.bounds.bottom / scaleToApply)
            }
        }
    };
    ...
    var _generateLayerInfo = function (layer, cb) {
        var htmlCenter = {
            x: Math.round((layer.calculatedDimensions.x + layer.calculatedDimensions.width / 2) / scaleToApply),
            y: Math.round((layer.calculatedDimensions.y + layer.calculatedDimensions.height / 2) / scaleToApply)
        };
        var imageStackMeta = {
            info: {
            ...
            properties: {
                "frame-size": {
                    width: Math.round(layer.calculatedDimensions.width / scaleToApply),
                    height: Math.round(layer.calculatedDimensions.height / scaleToApply)
                },
